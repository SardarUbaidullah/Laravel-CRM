...
@php
    ...
    if (method_exists($task, 'timeLogs') && $task->relationLoaded('timeLogs')) {
        $totalTimeSpent = $task->timeLogs->where('is_running', false)->sum('duration_minutes');
                        ...
                        <div class="flex items-start justify-between">
                            ...
                            <div class="flex items-start space-x-4 flex-1">
                                ...
                                <div class="flex items-center space-x-3">
                                    @if($task->status !== 'done')
                                    <button class="complete-task-btn w-8 h-8 border-2 border-gray-300 rounded flex items-center justify-center hover:border-green-500 hover:bg-green-50 transition-colors"
                            ...
                            <div class="flex flex-col items-end space-y-2 ml-4">
                                ...
                                <div class="flex items-center space-x-2">
                                    ...
                                    @if($task->status !== 'done')
                                    <button class="timer-btn w-8 h-8 rounded flex items-center justify-center transition-colors
                                        ...
                                        <i class="fas {{ $task->active_timer ? 'fa-stop' : 'fa-play' }} text-xs"></i>
                                    </button>
                                    @endif
    ...
    async function startTimer(taskId, taskTitle, button) {
        try {
            ...
            if (data.success) {
                ...
                timerDisplay.innerHTML = '<i class="fas fa-clock mr-1"></i><span class="timer-value">00:00:00</span>';
